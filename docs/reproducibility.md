...
## Archival environment gate

Status: `ADVANCED_BUT_STILL_OPEN`

The public package now has a cross-platform doctor, Linux CPython 3.12 CI, declared dependency
ranges, and exact software versions embedded in frozen phase receipts. Those controls verify the
current public evidence but are not yet an archival dependency lock.

Before any archival-release decision, create a coordinate-safe environment specification that
pins direct and transitive packages for the reference CPython 3.12 platform, records operating
system and CPU/GPU assumptions, and identifies which private-data steps cannot run in a public
checkout. A second authorized person must install and verify it from a clean checkout, then record
hashes and failures without regenerating or replacing spent-test results. A container or lockfile
is an implementation choice for that later gate, not selected here.
