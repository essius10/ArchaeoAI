...
$phase4CFiles = @(
    'docs/review/REVIEWER_GUIDE.md',
    'docs/review/EXTERNAL_REVIEW_NOTE.md',
    'docs/review/REVIEW_CHECKLIST.md',
    'docs/review/CLEAN_ENVIRONMENT_REPRODUCTION.md',
    'docs/review/READINESS_AUDIT.md',
    'research-log/2026-08-30-phase-4c-review-readiness.md'
)
foreach ($phase4CFile in $phase4CFiles) {
    if (-not (Test-Path -LiteralPath $phase4CFile -PathType Leaf)) {
        throw "Required Phase 4C review artifact missing: $phase4CFile"
    }
}
$phase4CText = (Get-Content -Raw $phase4CFiles) -join "`n"
if (
    $phase4CText -notmatch 'READY_FOR_EXTERNAL_REVIEW' -or
    $phase4CText -notmatch 'PARTIALLY REPRODUCIBLE — PRIVATE DATA REQUIRED FOR SPECIFIC STEPS' -or
    $phase4CText -match '(?i)["'']?(?:easting|northing|latitude|longitude|heritage_id|sample_id|pair_id)["'']?\s*[:=]\s*[-+]?\d'
) {
    throw 'Phase 4C readiness, reproduction, or coordinate-safety boundary failed.'
}
